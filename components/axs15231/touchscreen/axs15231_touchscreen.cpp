...
namespace {
  ...
  constexpr static const uint8_t AXS_READ_TOUCHPAD[8] = {0xb5, 0xab, 0xa5, 0x5a, 0x00, 0x00, 0x00, 0x08};
} // anonymous namespace

bool WriteC8D8(uint8_t c, uint8_t d, esphome::i2c::I2CDevice *device) {
  uint8_t data[2] = {c, d};
  auto err = device->write(data, 2, true);
  if (err != esphome::i2c::ERROR_OK) {
    ESP_LOGE(TAG, "Failed to write bytes c=0x%02X, d=0x%02X", c, d);
    return false;
  }
  return true;
}
...
bool IIC_WriteC8D8(uint8_t device_address, uint8_t c, uint8_t d, esphome::i2c::I2CDevice *device) {
  ...
  if (!WriteC8D8(c, d, device)) {
    ESP_LOGE(TAG, "Failed to write to device 0x%02X", device_address);
    return false;
  }
  return true;
}
...
void AXS15231Touchscreen::update_touches() {
  ...
  if (err != i2c::ERROR_OK) {
    ESP_LOGE(TAG, "Failed to write touchpad read command");
    return;
  }
